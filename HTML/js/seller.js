...
function listSellerOrders()
{
    ...
    $.when($.post('/composer/client/getMyOrders', options)).done(function(_results){
        if (_results.orders.length < 1) {
            $('#sellerOrderDiv').empty(); 
            $('#sellerOrderDiv').append(formatMessage(textPrompts.orderProcess.s_no_order_msg+options.id));
        }else{
            formatSellerOrders($('#sellerOrderDiv'), _results.orders);
        }
    });
}
